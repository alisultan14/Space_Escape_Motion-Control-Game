class Asteroid {
  ...
  Asteroid(float tempXpos, float tempYpos, float tempSpeedx, color tempR, color tempG, color tempB, float tempSize) {
  ...
  void display() {  //method to display 
    ...
    if (xpos-size/2<=width/8+50/2 && xpos-size/2>=width/8-25 && ypos+size/2>=mouseY-16/2 && ypos-size/2<=mouseY+16/2) {
      println(score);
      //score=0;
      crash=true;
      if (!boom.isPlaying()) {
